...
def run_ingestion():
    ...
    if (query_result is not None):
        try:
            ...
            flatten_json_data = RawToDF.get_flatten_dayforce_employee_properties_data(loads_json
                                                                  ,option_value_list
                                                                  ,properties_list)
            ...
            if (not df.empty):
                ...
                processed_writer.redshift_copy(df=df, seq_number=seq_number, ingestion_date=ingestion_date, mode="upsert", dtypes_col_list=dtypes_col_list,primary_keys=["employee_number"])
